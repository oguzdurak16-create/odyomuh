  ...
  return (
    <div className="odyssey-home">
      <section className="odyssey-hero">
        <div className="odyssey-aurora" aria-hidden="true" /><div className="odyssey-grid" aria-hidden="true" />
        <div className="odyssey-copy">
          ...
          <form className="odyssey-search" action="/search" method="get"><span aria-hidden="true">⌕</span><input name="q" type="search" placeholder="Bir uygarlık, olay veya kişi ara..." aria-label="Arşivde ara" /><button type="submit">Keşfet</button></form>
        ...
        {lead && <a className="odyssey-feature" href={lead.primaryPath}><img src={lead.image} alt={lead.title} width="1672" height="941" fetchPriority="high" decoding="async" /><div className="odyssey-feature-shade" /><div className="odyssey-feature-top"><span>Yeni keşif</span><time dateTime={lead.published}>{formatDate(lead.published)}</time></div><div className="odyssey-feature-copy"><small>{lead.labels?.[0] || 'Araştırma dosyası'}</small><h2>{lead.title}</h2><p>{lead.description}</p><strong>Dosyaya gir <b>↗</b></strong></div><div className="odyssey-orbit" aria-hidden="true"><i/><i/><i/></div></a>}
      </section>
