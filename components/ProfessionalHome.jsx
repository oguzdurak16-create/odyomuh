  ...
  return (
    <div className="odyssey-home">
      <section className="odyssey-hero">
        ...
        <div className="odyssey-copy">
          ...
          <form className="odyssey-search" action="/search" method="get">
            ...
            <input name="q" type="search" placeholder="Bir uygarlık, olay veya kişi ara..." aria-label="Arşivde ara" />
            <button type="submit">Keşfet</button>
          </form>
          <div className="odyssey-actions">
            <a href="#son-arastirmalar">Son araştırmalar</a>
            <a href="/p/tarih-kronolojisi.html">Zaman çizelgesini aç <span>↗</span></a>
          </div>
          <div className="odyssey-metrics">
            <div><strong>{allPosts.length}+</strong><span>Kaynaklı dosya</span></div>
            <div><strong>{labelList.length}</strong><span>Araştırma alanı</span></div>
            <div><strong>2</strong><span>Dil seçeneği</span></div>
          </div>
        </div>

        {lead && (
          <a className="odyssey-feature" href={lead.primaryPath}>
            <img src={lead.image} alt={lead.title} width="1672" height="941" fetchPriority="high" decoding="async" />
            <div className="odyssey-feature-shade" />
            <div className="odyssey-feature-top">
              <span>Yeni keşif</span>
              <time dateTime={lead.published}>{formatDate(lead.published)}</time>
            </div>
            <div className="odyssey-feature-copy">
              <small>{lead.labels?.[0] || 'Araştırma dosyası'}</small>
              <h2>{lead.title}</h2>
              <p>{lead.description}</p>
              <strong>Dosyaya gir <b>↗</b></strong>
            </div>
      ...
      <section className="odyssey-era-section">
        <div className="odyssey-section-head">
          <div><span>Zaman koridoru</span><h2>İnsanlığın kırılma noktaları</h2></div>
          <p>On iki bin yıllık geçmişi dönemler, uygarlıklar ve dönüm noktaları üzerinden tarayın.</p>
        </div>
        <div className="odyssey-era-track">
          {eras.map((era) => (
            <a href={era.href} key={era.title} className="odyssey-era-card">
              <div className="odyssey-era-icon">{era.icon}</div>
              <time>{era.year}</time>
              <h3>{era.title}</h3>
              <p>{era.text}</p>
              <span>Keşfet ↗</span>
      ...
      <section className="odyssey-research" id="son-arastirmalar">
        <div className="odyssey-section-head">
          <div><span>{formatDate(currentUpdateDate)} · Canlı arşiv</span><h2>Son araştırmalar</h2></div>
          <a href="/arsiv">Tüm dosyalar ↗</a>
        </div>
        <div className="odyssey-research-grid">
          {recentResearch.slice(0, 4).map((post, index) => (
            <a className={`odyssey-research-card ${index === 0 ? 'is-large' : ''}`} href={post.primaryPath} key={post.id}>
              <img src={post.image} alt={post.title} width="1672" height="941" loading={index ? 'lazy' : 'eager'} decoding="async" />
              <div className="odyssey-card-glass">
      ...
      <section className="odyssey-portals">
        {portals.map((portal) => (
          <a href={portal.href} className="odyssey-portal" key={portal.title}>
            <img src={portal.image} alt="" width="1672" height="941" loading="lazy" decoding="async" />
            <div><span>{portal.meta}</span><h3>{portal.title}</h3><p>{portal.text}</p><strong>Portalı aç ↗</strong></div>
          </a>
        ))}
      </section>

      <section className="odyssey-editorial">
        <div className="odyssey-section-head"><div><span>Editör seçkisi</span><h2>Derin okumalar</h2></div><p>Arşivden seçilmiş kapsamlı tarih, arkeoloji ve kültür dosyaları.</p></div>
        <div className="home-picks-grid">{editorPicks.map((post) => <PostCard post={post} key={post.id} />)}</div>
      </section>

      <section className="odyssey-latest">
        <div className="odyssey-section-head"><div><span>Arşiv akışı</span><h2>Keşfetmeye devam edin</h2></div><a href="/arsiv">Arşive git ↗</a></div>
