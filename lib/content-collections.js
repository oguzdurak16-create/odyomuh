...
import { applyAuthorityOverride } from '../data/authority-overrides.js';
...
function applyOverrides(items = []) {
  return items.map(applyContentOverride).map(applyTrafficOverride).map(applyAuthorityOverride).map(applyEditorialRouteOverride);
}
