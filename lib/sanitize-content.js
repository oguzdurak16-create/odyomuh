...
function cleanLegacyNoise(value = '') {
  return String(value || '')
    ...
    // Remove the legacy autogenerated study-method tail. The heading marks
    // the point where topic-specific editorial content ends and generic,
    // repeated exam/reading advice begins.
    .replace(/<h2[^>]*>[^<]*Konuyu daha derin okumak[^<]*<\/h2>[\s\S]*$/gi, '')
...
export function sanitizeContent(html = '', { imageAlt = 'ODYOMUH tarih görseli' } = {}) {
  return sanitizeHtml(cleanLegacyNoise(html), {
    ...
    transformTags: {
      // Article/page shells already render the document H1. Legacy imported
      // content occasionally contains another H1, so demote it to preserve
      // a clean, single-H1 hierarchy site-wide.
