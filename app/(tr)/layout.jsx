...
export default function TurkishRootLayout({ children }) {
  ...
  return <html lang="tr" suppressHydrationWarning style={{ '--font-cinzel': '"Cinzel", Georgia, serif', '--font-merriweather': '"Merriweather", Georgia, serif', '--font-inter': '"Inter", Arial, sans-serif' }}>
    ...
    <body><AnalyticsBootstrap /><HomeImageDeduplicator /><SiteChrome site={{ name: site.name, description: siteDescription }} pages={pageLinks} labels={labelLinks}><ConsentRestore />{children}<AdSlot slot={process.env.NEXT_PUBLIC_ADSENSE_CONTENT_SLOT} /></SiteChrome></body>
  </html>;
}
